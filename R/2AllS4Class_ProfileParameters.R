...
setClass("ChIPScore",
    ...
    validity = function(object) {
        if (!is(object@scores, "list")) {
            stop("ChIPScore score is not a list")
        }

        if (!is.null(object@loci) & !is(object@loci, "GRanges")) {
            stop("loci is not empty but it ain't no GRanges")
        }
...
genomicProfiles <- function(..., parameterOptions = NULL, genomicProfiles = NULL, ChIPScore = NULL) {
    ...
    PO <- c(
        "ploidy", "boundMolecules", "backgroundSignal", "maxSignal", "lociWidth",
