...
.computePWM <- function(PFM, pseudocount, BPFrequency, naturalLog, noOfSites) {
    if (min(PFM) < 0) {
        ...
        PWM <- .computePWMFromPFM(
            PFM, max(noOfSites), pseudocount,
...
### return: a 4 row matrix of the PWM, with rows: A, C, G, T
.computePWMFromPFM <- function(
  PFM, noOfSites, pseudocount,
  BPFrequency, naturalLog
) {
...
.parsePFMFromFile <- function(fileame) {
    ...
    if (!is.null(sequences) & length(sequences) > 0) {
        PFMExtended <- consensusMatrix(sequences,
            as.prob = TRUE,
            baseOnly = TRUE
        )
        PFM <- PFMExtended[seq_len(4), ]
    } else {
        PFM <- NULL
    }
    return(PFM)
...
.scoreDNAStringSet <- function(
  PWM, DNASequenceSet, strand = "+",
  strandRule = "max"
) {
...
# Translate the package's `cores` argument into a BiocParallel back end.
#
# The package used parallel::mclapply/mcmapply throughout, which is fork-based:
# on Windows it silently runs serially, so users there got no speedup and no
# warning. BiocParallel is portable, so the back end is chosen per platform:
#
#   cores < 2        -> SerialParam, matching mc.cores = 1 (the default), and
#                       avoiding worker start-up cost for the serial case
#   Windows          -> SnowParam (PSOCK); MulticoreParam is not supported
#                       there and would itself degrade to serial
#   everything else  -> MulticoreParam, i.e. the fork behaviour we had before
#
# Note SnowParam workers are separate processes that do not inherit the
# caller's globals, so anything passed to bplapply/bpmapply must be
# serialisable -- package internals and their arguments are, but any future
# closure over a large or connection-backed object will not be.
.bpParam <- function(cores = 1L) {
    if (is.null(cores) || length(cores) != 1L || is.na(cores) || cores < 2L) {
        return(BiocParallel::SerialParam())
    }
    if (.Platform$OS.type == "windows") {
        return(BiocParallel::SnowParam(workers = cores))
    }
    BiocParallel::MulticoreParam(workers = cores)
}

# Shared validity check for the genomicProfiles / parameterOptions argument
# pair, which every compute* entry point takes. parameterOptions is optional
# throughout the package, so NULL is always accepted.
.validateGenomicProfiles <- function(genomicProfiles, parameterOptions = NULL) {
    # Resolve the argument names the *user* typed, one frame up (the compute*
    # call), rather than this function's own formal names -- otherwise every
    # error would read "genomicProfiles is not a ..." regardless of what the
    # caller passed. Must stay inline: moving this into a helper adds a call
    # frame and breaks the -1L offset.
    m <- tryCatch(match.call(sys.function(-1L), sys.call(-1L)),
        error = function(e) NULL
    )
    nm <- function(arg) {
        e <- if (is.null(m)) NULL else m[[arg]]
        if (is.null(e)) arg else paste(deparse(e), collapse = "")
    }
    if (!.is.genomicProfiles(genomicProfiles)) {
        stop(nm("genomicProfiles"),
            " is not a genomicProfiles object.",
            call. = FALSE
        )
    }
    if (!is.null(parameterOptions) &&
        !.is.parameterOptions(parameterOptions)) {
        stop(nm("parameterOptions"),
...
.computeBPFrequency <- function(object) {
    ...
    if (length(object) > 1) {
        BPFrequency <- apply(
            (alphabetFrequency(object, baseOnly = TRUE)[, seq_len(4)]),
            2, sum
        )
        BPFrequency <- BPFrequency / sum(as.numeric(BPFrequency))
    } else {
        BPFrequency <- (alphabetFrequency(object, baseOnly = TRUE)[, seq_len(4)])
        BPFrequency <- BPFrequency / sum(as.numeric(BPFrequency))
    }
    return(BPFrequency)
}
...
.generateChIPProfileRcpp <- function(
  inputVector, mean, sd, smooth = NULL,
  norm = TRUE, peakSignificantThreshold = NULL
    ...
    kernel <- c(
        rev(bigF[seq(2, peakSignificantSize)]),
    ...
    if (!is.null(smooth)) {
        ...
        peaks <- roll_mean(c(
            rep(0, times = leftKernel), peaks,
            rep(0, times = rightKernel)
        ), smooth, weights = rep(1, smooth))
    }
...
.generateChIPProfile <- function(
  ...
  smooth = NULL, norm = TRUE, quick = TRUE, peakSignificantThreshold = NULL
    ...
    sp <- c(
        rev(gamma[seq(2, peakSignificantSize)]),
    ...
    if (!quick) {
        for (peakCentre in peak.centres) {
            thisPeak <- vector("numeric", l)
            thisPeak[peakCentre:l] <- gamma[seq_len(l - peakCentre + 1)]
            if (peakCentre != 1) {
                thisPeak[seq_len(peakCentre - 1)] <- gamma[peakCentre:2]
            }
            peaks <- peaks + thisPeak * inputVector[peakCentre]
        }
    } else {
        for (peakCentre in peak.centres) {
            thisPeak <- sp
            ...
            if (cutLeft != 0) {
                thisPeak <- thisPeak[-(seq_len(cutLeft + 1))]
            }
            if (cutRight != 0) {
            ...
            peaks[left:right] <- peaks[left:right] + thisPeak * inputVector[peakCentre]
        }
    }
...
searchSites <- function(
  ...
  BoundMolecules = "all", Locus = "all"
    ...
    if (!is.null(names(Sites)) & all(names(Sites) %in% c("Optimal", "Occupancy", "ChIPProfiles", "goodnessOfFit"))) {
        bufferSites <- list(
            "Optimal" = Sites[[1]],
            "Occupancy" = searchSites(unname(Sites$Occupancy),
                lambdaPWM = lambdaPWM,
                BoundMolecules = BoundMolecules,
                Locus = Locus
            ),
            "ChIPProfiles" = searchSites(unname(Sites$ChIPProfile),
                lambdaPWM = lambdaPWM,
                BoundMolecules = BoundMolecules,
                Locus = Locus
            ),
            "goodnessOfFit" = searchSites(unname(Sites$goodnessOfFit),
                lambdaPWM = lambdaPWM,
                BoundMolecules = BoundMolecules,
                Locus = Locus
            )
        ...
        if (all(lambdaPWM == "all")) {
            ...
            localNames <- vapply(strsplit(vapply(
                strsplit(names(bufferSites), " &"),
                "[[", character(1), 1
            ), lambda), "[[", character(1), 2)
            for (i in seq_along(lambdaPWM)) {
                buffer <- c(buffer, grep(
                    paste0("^", lambdaPWM[i], "$"),
                    localNames
                ))
            }
        ...
        if (all(BoundMolecules == "all")) {
            ...
            localNames <- vapply(strsplit(names(bufferSites), bound), "[[", character(1), 2)
            for (j in seq_along(BoundMolecules)) {
                buffer <- c(buffer, grep(
                    paste0("^", as.character(as.integer(BoundMolecules[j])), "$"),
...
.AccessExtract <- function(subject, query, isCS = FALSE) {
    ...
    for (i in seq_along(subject)) {
        ...
        if (isCS) {
            setLocal[[i]] <- data.frame(
                "chr" = as.character(seqnames(localIntersect)),
...
.cleanGoF <- function(genomicProfiles, ValidationSet, stepSize = 10) {
    ...
    setUpGoF <- lapply(genomicProfiles, function(gp, chip, stepSize) {
        ...
        validation <- mapply(function(x, idx) {
            return(x[idx])
        }, x = chip, idx = idx, SIMPLIFY = FALSE)
        ## internalCheck
        internalCheck <- all.equal(vapply(predicted, length, integer(1)), vapply(validation, length, integer(1)))
        if (!internalCheck) {
            stop("predicted and valdiation set are not the same length")
        }

        paralleValues <- mapply(
            function(x, y) {
...
.meanGoFScore <- function(GoF) {
    ...
    meanGoF <- vapply(idx, function(idx, GoF) {
        res <- mean(vapply(GoF, "[[", numeric(1), idx), na.rm = TRUE)
        return(res)
    }, numeric(1), GoF = GoF)
...
.peakExtractionFScore <- function(predicted, locusProfile, region = TRUE) {
    ...
    if (low == 0) {
        ...
        subs <- vapply(subs, "^", numeric(1), 2)[seq(2, 21)]
    } else {
        subs <- seq(low, high, length.out = 20)
        subs <- vapply(subs, "^", numeric(1), 2)
    }
    ...
    prec <- mean(vapply(performance(pred, "prec")@y.values, mean, numeric(1), na.rm = TRUE))
    rec <- mean(vapply(performance(pred, "rec")@y.values, mean, numeric(1), na.rm = TRUE))
    fscore <- mean(vapply(performance(pred, "f")@y.values, mean, numeric(1), na.rm = TRUE))
    acc <- mean(vapply(performance(pred, "acc")@y.values, mean, numeric(1), na.rm = TRUE))
    mcc <- mean(vapply(performance(pred, "mat")@y.values, mean, numeric(1), na.rm = TRUE))

    auc <- mean(vapply(performance(pred, "auc")@y.values, mean, numeric(1), na.rm = TRUE))
...
.OptimalFormatConversion <- function(goodnessOfFit) {
    ...
    convertedFormat <- lapply(fitVec, function(fitVec, lociVec, paramVec, goodnessOfFit) {
        param <- lapply(paramVec, function(paramVec, fitVec, lociVec, goodnessOfFit) {
            loci <- vapply(goodnessOfFit[[paramVec]], "[[", numeric(1), fitVec)
            return(loci)
...
.optimalExtraction <- function(goodnessOfFit, rank = FALSE) {
    ...
    if (rank) {
        ...
        optimalParama <- mapply(function(met, tag) {
            ...
            return(optimal)
        }, met = metrics, tag = tag, SIMPLIFY = FALSE)
        ## building matricies
        mattemp <- matrix(0, ncol = length(bm), nrow = length(lambda))
        rownames(mattemp) <- lambda
        colnames(mattemp) <- bm
...
.unpackMetrics <- function(metrics) {
    lambda <- "lambda = "
    bound <- "boundMolecules = "
    lambdaLoc <- vapply(strsplit(vapply(
        strsplit(names(metrics), " &"),
        "[[", character(1), 1
    ), lambda), "[[", character(1), 2)
    bmLoc <- vapply(strsplit(vapply(
        strsplit(names(metrics), " &"),
        "[[", character(1), 2
    ), bound), "[[", character(1), 2)
...
.what.is.predictedProfile <- function(predictedProfile) {
    ...
    if (is(predictedProfile, "list") &
        ...
        predictedProfile <- lapply(predictedProfile, function(x) {
            return(x$ChIP)
        })
        ...
        loci <- GRanges(
            seqnames = as.character(predictedProfile),
