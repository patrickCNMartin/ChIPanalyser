...
.formatCheck <- function(profile, type) {
    if (type == "ChIP") {
        if (!is.null(colnames(profile))) {
            cols <- c(
                grep("chr", colnames(profile), ignore.case = TRUE),
            ...
            profile <- GRanges(
                seqnames = as.character(profile[, 1]),
...
.internalChIPExtraction <- function(profile, loci, peaks = NULL, chromatinState = NULL,
    ...
    ExtractedProfiles <- unlist(BiocParallel::bplapply(localRanges,
        FUN = .extractionChIP,
    ...
    if (!is.null(maxSignal) & noiseFilter != "sigmoid") {
        ExtractedProfiles <- BiocParallel::bplapply(ExtractedProfiles, function(x, maxSignal) {
            x / maxSignal
        }, maxSignal, BPPARAM = .bpParam(cores))
    } else if (!is.null(maxSignal) & noiseFilter == "sigmoid") {
        maxSignal <- max(vapply(ExtractedProfiles, max, numeric(1)))
        ExtractedProfiles <- BiocParallel::bplapply(ExtractedProfiles, function(x, maxSignal) {
            x / maxSignal
        }, maxSignal, BPPARAM = .bpParam(cores))
    }
    ...
    if (!is.null(reduce)) {
        ...
        topScore <- vapply(ExtractedProfiles, max, numeric(1))
        ...
        chr <- vapply(strsplit(GenomicRange, ":"), "[[", character(1), 1)
        start <- vapply(strsplit(vapply(strsplit(GenomicRange, ":"), "[[", character(1), 2), "\\.."), "[[", character(1), 1)
        end <- vapply(strsplit(vapply(strsplit(GenomicRange, ":"), "[[", character(1), 2), "\\.."), "[[", character(1), 2)
    ...
    if (!is.null(chipSmooth)) {
        ExtractedProfiles <- BiocParallel::bplapply(ExtractedProfiles, .vectorSmooth,
            chipSmooth, TRUE,
            BPPARAM = .bpParam(cores)
        )
    }
    ...
    areThereNAs <- which(vapply(ExtractedProfiles, function(x) {
        sum(is.na(x))
    }, integer(1)) > 0)
