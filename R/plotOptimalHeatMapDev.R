plotOptimalHeatMaps <- function(optimalParam, contour = TRUE, col = NULL, main = NULL, layout = TRUE, overlay = FALSE) {
    ...
    nans <- lapply(optimalParam, function(x) {
    ...
    if (overlay) {
        ...
        for (k in seq_along(optimalParam)) {
            ...
            if (namevec[k] %in% c("geometricMean", "MSEMean", "ksMean", "recallMean")) {
                ord <- order(buffer, decreasing = FALSE)
            } else {
                ord <- order(buffer, decreasing = TRUE)
            }
            ...
            topHits <- which(optimalMatrix < max(top), arr.ind = TRUE)
    ...
    ## plotting
    if (layout) {
        layout(matrix(seq_len(2), ncol = 2, byrow = TRUE), width = c(6, 1), height = c(1, 1))
    }
    ...
    for (i in seq_along(optimalParam)) {
        ...
        graphics::image(seq_along(xlabs), seq_along(ylabs), t(optimalParam[[i]]),
        ...
        axis(1, at = seq_along(xlabs), labels = FALSE)
        text(seq_along(xlabs), y = (-0.2), srt = 45, adj = 1, labels = xlabs, xpd = TRUE, cex = 1.15)
        axis(LEFT <- 2, at = seq_along(ylabs), labels = ylabs, las = HORIZONTAL <- 1, cex.axis = 1.15)
        if (contour) {
            contour(seq_along(xlabs), seq_along(ylabs), t(
                optimalParam[[i]]
            ),
            nlevels = 7, drawlabels = FALSE,
