...
computeGenomeWideScores <- function(genomicProfiles, DNASequenceSet,
    ...
    if (!is(DNASequenceSet, "BSgenome") &
        ...
        stop(
            deparse(substitute(DNASequenceSet)),
    ...
    if (!is(chromatinState, "GRanges") && !is.null(chromatinState)) {
        stop(chromatinState, " must be a GRanges Object.")
    }
    ...
    if (!is.null(chromatinState)) {
        ...
        DNASequenceSet <- BiocParallel::bpmapply(.internalDNASequenceFromAccess,
            DNA = DNA, Access = Access, BPPARAM = .bpParam(cores)
        )
    ...
    DNASequenceScoreSetTotalAcces <- BiocParallel::bplapply(DNASequenceSet,
        .scoreDNAStringSet,
        PWM = PWMMat,
        strand = strand, strandRule = strandRule, BPPARAM = .bpParam(cores)
    )
