...
computePWMScore <- function(
  ...
  loci = NULL, chromatinState = NULL, parameterOptions = NULL, cores = 1, verbose = TRUE
    ...
    if (!is(chromatinState, "GRanges") && !is.null(chromatinState)) {
        stop(
            deparse(substitute(chromatinState)),
    ...
    if (length(loci) >= cores & cores > 1) {
        ...
        Scores <- BiocParallel::bplapply(localSequence, .internalPWMScoreExt,
    ...
    lociDrop <- names(AccessibleSequence)[which(vapply(AccessibleSequence, length, integer(1)) == 0)]
