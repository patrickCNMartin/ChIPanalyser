...
.splitRanges <- function(ranges, cores) {
    if (cores > 1 & cores < length(ranges)) {
        ...
        start <- SplitRanges[seq_len(length(SplitRanges) - 1)]
...
.extractionChIP <- function(profile, loci, noiseFilter = "sigmoid") {
    ...
    if (grepl("mean", noiseFilter, ignore.case = TRUE)) {
        Background <- mean(vapply(localProfiles, mean, numeric(1)), na.rm = TRUE)
    }

    if (grepl("median", noiseFilter, ignore.case = TRUE)) {
        Background <- median(vapply(localProfiles, median, numeric(1)), na.rm = TRUE)
    }

    if (grepl("sigmoid", noiseFilter, ignore.case = TRUE)) {
        Background <- 0
        midpoint <- mean(vapply(localProfiles, quantile, numeric(1), 0.95), na.rm = TRUE)
        localProfiles <- lapply(localProfiles, .sigmoidWeight, midpoint = midpoint)
    }
    ## Actually applying the noise filter
    localProfiles <- lapply(localProfiles, function(x) {
        x[x < Background] <- 0
        return(x)
    })
    if (is.null(names(loci))) {
        names(localProfiles) <- paste0(
            as.character(seqnames(loci)),
            ":", start(loci), "..", end(loci)
        )
    } else {
...
.internalPWMScoreExt <- function(
  loci, DNASequenceSet, chromatinState, PWM, PWMThresholdLocal,
  minPWMScore, maxPWMScore, strand, strandRule
    ...
    for (i in seq_along(loci)) {
        ## ID matching
        localDNASequenceSet <- DNASequenceSet[unique(!is.na(match(
            as.character(seqnames(loci))[i],
        ...
        ]], start = start(loci[i]), end = end(loci[i]))
...
##### computeChipProfile #####
### Looping over Loci###
.internalChIPLoci <- function(
  profile, Occup, LocalSet, OccupancyVals, chipMean = chipMean, chipSd = chipSd,
...
.internalChIPParam <- function(
  SplitGRList, Occup, LocalSet, OccupancyVals, chipMean = chipMean, chipSd = chipSd,
  stepSize = stepSize, norm = norm, chipSmooth = chipSmooth, peakSignificantThreshold = peakSignificantThreshold, ZeroBackground = ZeroBackground, removeBackground = removeBackground, method = method
) {
...
.GoFLoci <- function(GoF, method = "all", step = 10, cores = 1) {
    localGoF <- BiocParallel::bplapply(GoF, function(x, method, step) {
        return(.methodSwitchGoF(x$prediction, x$validation, method, step))
    }, method = method, step = step, BPPARAM = .bpParam(cores))
    return(localGoF)
}

.methodSwitchGoF <- function(gp, chip, method, step) {
    if (any(grepl(method, c("recall", "precesion", "fscore", "MCC", "Accuracy", "AUC"), ignore.case = TRUE))) {
        method <- "all"
    }
