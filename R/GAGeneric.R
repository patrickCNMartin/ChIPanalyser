...
.initiateGASolutions <- function(solutions) {
    strict <- all(vapply(solutions, function(x) {
...
.setStrictSoltions <- function(solutions, param) {
    ...
    if (grepl("boundMolecules", getName, ignore.case = TRUE) |
        grepl("Bound molecules", getName, ignore.case = TRUE) |
        grepl("N", getName, ignore.case = TRUE)) {
        solutions <- c(
            1, 10, 20, 50, 100,
            200, 500, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 10000, 20000, 50000, 100000,
            200000
        )
    } else if (grepl("ScalingFactor", getName, ignore.case = TRUE) |
        grepl("Scaling Factor", getName, ignore.case = TRUE) |
        grepl("lambda", getName, ignore.case = TRUE)) {
        solutions <- c(0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.5, 3, 3.5, 4, 4.5, 5)
    } else if (grepl("CS", getName, ignore.case = TRUE) |
        grepl("DNAAccessibility", getName, ignore.case = TRUE) |
        grepl("DNAAffinity", getName, ignore.case = TRUE)) {
        solutions <- seq(0, 1, by = 0.2)
    } else if (grepl("PWMThreshold", getName, ignore.case = TRUE) |
        grepl("PWM Threshold", getName, ignore.case = TRUE)) {
...
.generateRandomGASolution <- function(population) {
    if (is.list(population[[1]])) {
        samples <- lapply(population, function(x) {
            res <- lapply(x, sample, size = 1)
            res$fitness <- NA
            res$computed <- FALSE
            res$gen <- 1
            return(res)
        ...
        samples <- lapply(population, function(x) {
            res <- as.list(apply(x, 2, sample, size = 1))
            res$fitness <- NA
            res$computed <- FALSE
            res$gen <- 1
            return(res)
...
.fittyMacFitFace <- function(population, method, fit, generation) {
    ...
    Fit <- vapply(population, function(x) {
        x$fitness
    }, numeric(1))
...
.buildDatabase <- function(population, database, new = FALSE, generation = 1) {
    if (new == FALSE) {
        ...
        return(list(
            "DataBase" = as.data.frame(database), "ComputedPop" = ComputedPop,
...
.crossover <- function(ga1, ga2, extent = "random") {
    ...
    if (extent == "random") {
        ...
        if (extent < (1 / length(ga1))) {
            stop("Extent argument out of bounds - Limited by Number of parameters")
        }
