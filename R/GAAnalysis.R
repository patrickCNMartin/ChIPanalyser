...
evolve <- function(population, DNASequenceSet, ChIPScore,
    ...
    if (!is.null(chromatinState) & length(grep("cs", names(population[[1]]), ignore.case = TRUE)) > 1) {
        CS <- setChromatinStates(population, chromatinState)
    } else if (!is.null(chromatinState) & length(grep("cs", names(population[[1]]), ignore.case = TRUE)) == 1) {
    ...
    for (i in seq_len(generations)) {
        ...
        if (length(ToBeComputedPop) != 0) {
            ...
            if (!is.null(chromatinState) & length(grep("cs", names(ToBeComputedPop[[1]]), ignore.case = TRUE)) > 1) {
                CS <- setChromatinStates(ToBeComputedPop, chromatinState)
            } else if (!is.null(chromatinState) & length(grep("cs", names(ToBeComputedPop[[1]]), ignore.case = TRUE)) == 1) {
                CS <- list(chromatinState)
            } else {
            ...
            if (lambda == TRUE) {
                ToBeComputedPop <- BiocParallel::bpmapply(.individualSurvival, ToBeComputedPop, CS,
                    MoreArgs = list(genomicProfiles, DNASequenceSet,
                        ChIPScore = ChIPScore, fitness = method, lambdaDB = lambdaDB
                    ), SIMPLIFY = FALSE, BPPARAM = .bpParam(cores)
                )
            } else {
                ToBeComputedPop <- BiocParallel::bpmapply(.individualSurvival, ToBeComputedPop, CS,
                    MoreArgs = list(genomicProfiles, DNASequenceSet,
                        ChIPScore = ChIPScore, fitness = method, lambdaDB = NULL
                    ), SIMPLIFY = FALSE, BPPARAM = .bpParam(cores)
                )
            }
        ...
        message("Generation Fitness: ", GenFit[[i]])
...
getHighestFitnessSolutions <- function(population, child = 2, method = "geometric") {
    ...
    UFit <- unlist(vapply(population, "[[", numeric(1), "fitness"))

    if (grepl("ks", method, ignore.case = TRUE) |
        grepl("geometric", method, ignore.case = TRUE) |
        grepl("MSE", method, ignore.case = TRUE)) {
        TopFit <- order(UFit, decreasing = FALSE)[seq_len(child)]
    } else {
        TopFit <- order(UFit, decreasing = TRUE)[seq_len(child)]
    }
...
singleRun <- function(
  ...
  ChIPScore, fitness = "all"
    ...
    lambdaPWM(genomicProfiles) <- unlist(indiv[
        grepl("ScalingFactor", names(indiv), ignore.case = TRUE) |
            grepl("Scaling Factor", names(indiv), ignore.case = TRUE) |
            grepl("lambda", names(indiv), ignore.case = TRUE)
    ])
    # Setting PWMThreshold from GA population
    PWMThreshold(genomicProfiles) <- unlist(indiv[
        grepl("PWMThreshold", names(indiv), ignore.case = TRUE) |
            grepl("PWM Threshhold", names(indiv), ignore.case = TRUE)
    ])
    # Setting Number of bound molecules from GA population

    boundMolecules(genomicProfiles) <- unlist(indiv[
        grepl("boundMolecules", names(indiv), ignore.case = TRUE) |
            grepl("Bound molecules", names(indiv), ignore.case = TRUE) |
            grepl("^N$", names(indiv), ignore.case = TRUE)
    ])
...
setChromatinStates <- function(population, chromatinStates) {
    ...
    affinities <- lapply(population, function(x) {
        x[grep(pattern = "cs", names(x), ignore.case = TRUE)]
    })

    # selecting score in chromatin states GR
    if (length(grep(x = chromatinStates$name, pattern = "cs", ignore.case = TRUE)) < 1) {
        # stateNames<-
    ...
    # Removing levels
    chromatinStates <- GRanges(
        seqnames = as.character(seqnames(chromatinStates)),
...
generateStartingPopulation <- function(population, parameters, names = NULL) {
    ...
    if (!is.list(parameters)) {
        pop <- lapply(pop, function(x) {
            x <- vector("list", length(parameters))
            names(x) <- parameters
            return(x)
        })
    } else {
