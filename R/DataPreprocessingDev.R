...
processingChIP <- function(profile, loci = NULL, reduce = NULL,
    ...
    if (is.null(loci)) {
        ...
        localRanges <- lapply(localRanges, function(x) {
            x <- GRanges(
                seqnames = unique(as.character(seqnames(x))),
    ...
    ChIPProfile <- .internalChIPExtraction(
        profile = profile,
        loci = loci,
        peaks = peaks,
        chromatinState = chromatinState,
    ...
    if (!is.null(reduce)) {
        ChIPProfile <- .ChIPScore(
            scores = ChIPProfile[[1]], loci = ChIPProfile[[2]],
            maxSignal = max(vapply(ChIPProfile[[1]], max, numeric(1))),
            backgroundSignal = mean(vapply(ChIPProfile[[1]], mean, numeric(1))),
            lociWidth = lociWidth(parameterOptions),
            paramTag = "ChIPExt"
        )
    } else {
        ChIPProfile <- .ChIPScore(
            scores = ChIPProfile, loci = loci,
            maxSignal = max(vapply(ChIPProfile, max, numeric(1))),
            backgroundSignal = mean(vapply(ChIPProfile[[1]], mean, numeric(1))),
            lociWidth = lociWidth(parameterOptions),
            paramTag = "ChIPExt"
        )
