...
```{r load-data, eval=TRUE, warnings = FALSE, echo=TRUE,message=FALSE}
...
```{r process-chip, eval=TRUE, warnings = FALSE}
...
```{r genomic-profiles-from-pfm, eval =TRUE}
...
```{r genomic-profiles-from-pwm, eval=FALSE}
...
```{r compute-optimal, eval=TRUE,warnings=FALSE}
...
```{r default-optimal-parameters, eval=TRUE, echo=TRUE}
 ## Lambda Values
 seq(0.25,5,by=0.25)
...
```{r extract-optimal-parameters, eval =TRUE}
...
```{r plot-optimal-heatmaps, eval=TRUE, warnings = FALSE, fig.width=10, fig.height=8}
# Plotting Optimal heat maps
par(oma=c(0,0,3,0))
layout(matrix(1:8,ncol=4, byrow=TRUE),width=c(6,1.5,6,1.5),height=c(1,1))
...
```{r plot-occupancy-profile-optimal, eval=TRUE,fig.width=15, fig.height=8}
...
```{r parameter-options-setup, eval =TRUE}
...
```{r processing-chip-advanced, eval=FALSE}
...
```{r genomic-profiles-bpfrequency, eval=TRUE}
str(genomicProfiles())
...
```{r genomic-profiles-parsing-options, eval=FALSE}
...
```{r compute-optimal-custom-parameters, eval=FALSE}
...
```{r extract-plot-optimal-custom, eval=FALSE}
...
```{r single-parameter-combination, eval=TRUE,warnings=FALSE}
...
```{r plot-single-combination, eval=TRUE,fig.width=12, fig.height=4.5}
...
```{r parameter-options-summary, eval=TRUE,echo=TRUE}
parameterOptions()
```
...
```{r genomic-profiles-structure, eval=TRUE, echo=TRUE}
str(genomicProfiles())
```
...
```{r accessors-list, eval=FALSE, echo=TRUE}
...
factors from genome-wide binding profiles. Nucleic Acids Res., 43(1), 84–94.
https://doi.org/10.1093/nar/gku1269

Martin PCN, Zabet NR (2020) Dissecting the binding mechanisms of transcription
factors to DNA using a statistical thermodynamics framework. Comput. Struct.
Biotechnol. J., 18, 3590–3605. https://doi.org/10.1016/j.csbj.2020.11.006
