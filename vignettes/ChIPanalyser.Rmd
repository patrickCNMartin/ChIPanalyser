...
factors are proteins that bind to DNA in a site-specific fashion.
...
sequencing. This method generatesa genome wide profile with exhibiting peaks
...
At the core of this packageresides an approximation of statistical
thermo dynamcis as suggested by Zabet (Zabet et al. 2015).
The statistical thermo dynamcis framework proposed by Zabet offers a strong
...
stored in the package. This data represents minimal workable examples for
...
from the biological data used in this vignette. These values
...
`ScalingFactorPWM` (described in the model as lambda). In order to
backward infer these parameters, we suggest to use `computeOptimal`.
...
more magic to it. In the following step we will describe each of the functions.
...
`AllSitesAboveThreshold`. Now the Occupancy values for each sites is
included.
...
will compute ChIP-seq like score from occupancy values.
...
```
This function will return a list of `GRangesList` with ChIP-seq like scores
for every *n* base pairs (with n = stepSize, see bellow).
...
This section will described `ChIPanalyser` work flow. However in this section
we will describe in detail data objects, parameters, and functions.
Please refer to this section if in doubt to the use of any function or
parameter.
...
The out put of most functions will be a `genomicProfileParameters` object.
...
section **Work Flow - Analysis**. This object comes in the following form:
...
with the values that were assigned to each argument. Only two slots are
absolutely required in order to build a `genomicProfileParameters` object
...
it also possible to link towards (path/to/file) this file and
...
tailor the values assigned `genomicProfileParameters` to your needs.
...
The optimal value for `ScalingFactorPWM` is may be inferred by using
`computeOptimal`. Different values for `ScalingFactorPWM` will influence
the fitness of the model. For more information,
...
then *log2* will be used instead. Choosing between *natural logarithm*
or *log2* will depend on the *pseudocount* used. Keep in mind that,
...
* `strandRule`, indicates the how the genome should be scored with
...
Although it is still possible if absolutely required, we strongly advise
...
the function will won't return any value.
.
...
of the average PWM Score. This score depends on the value assigned to `lambda`.
It is possible to access this slot using:
...
`genomicProfileParameters` represent a good chunk of the parameters
...
As stated previously, we strongly advise using tailored parameters in order
to increase model fitness. It is especially the case here, as slot such as
...
(Default value is set at 1000). In this model, occupancy is reliant on the
number of bound molecules. The number of molecules will influence
the fitness of the model. It is possible to infer the number of
...
(replaced by correlation and MSE Threshold) as they represent
...
`DNAAccessibility` is an optinal argument in `computeGenomeWidePWMScore`.
If present **AND** `GenomeWide = FALSE`, then the genome will be scored
...
Finally, if you sequence of interest does not contain any accessible DNA,
...
see `computeOccupancy` example as we will demonstrate  multiple value
...
to improve the model's fitness. As a reminder, a `occupancyProfileParameters`
...
of GRanges with two metadata columns (PWMScore and Occupancy).
...
a GenomicRanges object with ChIP profile values as a metadata column.
...
(See stepSize slot in `occupancyProfileParameters`).
...
provide a way to compare both profiles.
...
over MSE ( extreme values are replaced by threshold). The goal is to fund
...
The last argument, `parameter` defines which metric you wish to compute.
...
# Plotting Results
As it the case in mamy fields, data visualisation is a key aspect
...
* `geneRef`, a GRangesList containing genetic information
